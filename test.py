...
# Dictionary to store the user's language choice
user_language = {}
...
async def send_info(message: types.Message):
    language = user_language.get(message.chat.id, "ru")  # Default to Russian if language is not set
    if message.text == "О нас" or message.text == "Biz haqimizda":
    ...
    elif message.text == "Новости" or message.text == "Yangiliklar":
    ...
    elif message.text == "Наши соцсети" or message.text == "Bizning ijtimoiy tarmoqlar":
        if language == "ru":
            await message.answer("Наши социальные сети:", reply_markup=get_social_links("ru"))
        else:
            await message.answer("Bizning ijtimoiy tarmoqlar:", reply_markup=get_social_links("uz"))
...
async def live_assistant(message: types.Message):
    language = user_language.get(message.chat.id, "ru")  # Default to Russian if language is not set
    if language == "ru":
        await message.answer("Подключаем вас к живому ассистенту. Пожалуйста, подождите...")
    else:
        await message.answer("Sizni jonli yordamchi bilan bog'layapmiz. Iltimos, kuting...")
    ...
    if language == "ru":
        await message.answer("Ваш запрос был отправлен. С вами скоро свяжется один из членов нашей команды.")
    else:
        await message.answer("Sizning so'rovingiz yuborildi. Tez orada bizning jamoamizdan kimdir siz bilan bog'lanadi.")
